    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "container_engine=info".into()),
        )
        .init();
...
async fn health(State(s): State<Arc<AppState>>) -> Json<Health> {
    ...
    Json(Health {
        ...
        total_ops: st.total_starts + st.total_deploys,
    })
}
...
async fn create_container(
    ...
    let name = req
        .name
        .unwrap_or_else(|| format!("container-{}", &uuid::Uuid::new_v4().to_string()[..8]));
    let vcpu = req.vcpu.unwrap_or(0.25);
    let mem = req.memory_mb.unwrap_or(512);
    {
        let mut st = s.stats.lock().unwrap();
        st.total_containers += 1;
        st.total_starts += 1;
    }
    Json(CreateContainerResponse {
...
async fn logs(State(_s): State<Arc<AppState>>, Json(req): Json<LogsRequest>) -> Json<LogsResponse> {
    ...
    let lines: Vec<String> = (0..count)
        .map(|i| {
            format!(
                "[2026-02-23T00:00:{}Z] INFO container runtime: request processed",
                i
            )
        })
        .collect();
...
async fn runtimes() -> Json<Vec<RuntimeInfo>> {
    Json(vec![
        RuntimeInfo {
            name: "alice-rt".into(),
            version: "1.0.0".into(),
            isolation: "cgroup-v2 + namespaces".into(),
            startup_ms: 45,
            description: "Rust-native container runtime with direct namespace control".into(),
        },
        RuntimeInfo {
            name: "alice-rt-wasm".into(),
            version: "0.5.0".into(),
            isolation: "WebAssembly sandbox".into(),
            startup_ms: 5,
            description: "WASM-based micro-isolation for serverless functions".into(),
        },
    ])
}
...
async fn stats(State(s): State<Arc<AppState>>) -> Json<StatsResponse> {
    let st = s.stats.lock().unwrap();
    Json(StatsResponse {
        total_containers: st.total_containers,
        total_starts: st.total_starts,
        total_stops: st.total_stops,
        total_deploys: st.total_deploys,
        vcpu_hours: st.vcpu_hours,
    })
}
